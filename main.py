...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='SelfieToAnime', help='dataset_name')
    ...
    ## 影响显存，原64通道虽意味着模型可以捕捉到更多的特征细节，具有更强的表达能力，但计算量和内存需求更大，训练时间更长，也需要更高的硬件资源
    parser.add_argument('--ch', type=int, default=32, help='base channel number per layer')
    ## 生成器残差块的数量
    parser.add_argument('--n_res', type=int, default=4, help='The number of resblock')
    ## 判别器的数量，原6层对图像特征进行更深入的分析，但计算量（显存）和内存需求更大，训练时间过长
